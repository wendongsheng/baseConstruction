...
- (instancetype)initWithItem:(LYAlbumItem *)item maxNum:(NSInteger)maxNum
{
    self = [super init];
    if (self) {
        self.albumItem = item;
...
- (void)photoButtonPress:(UIButton *)button event:(id)event
{
    ...
    if (indexPath)
    {
        ...
        {
            NSString *msg = [NSString stringWithFormat:@"最多只能上传%ld张图片！", (long)maxPhotoNumber];
...
{
    self.controlView.numLabel.text = [NSString stringWithFormat:@"%ld/%ld", (long)selectNum, (long)maxPhotoNumber];
...
- (instancetype)initWithFrame:(CGRect)frame maxNum:(NSInteger)maxNum
{
    ...
    if (self)
    {
        ...
        self.numLabel.text = [NSString stringWithFormat:@"0/%ld", (long)maxNum];
